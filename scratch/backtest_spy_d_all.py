...
for config in configs:
    ...
    for idx, date in enumerate(dataset_dates):
        ...
        strat_d1_daily_ret = weight_d1 * bench_daily_ret
        strat_d2_daily_ret = weight_d2 * bench_daily_ret
        
        bench_val *= (1.0 + bench_daily_ret)
        strat_a_val *= (1.0 + strat_a_daily_ret)
        strat_b_val *= (1.0 + strat_b_daily_ret)
        strat_d1_val *= (1.0 + strat_d1_daily_ret)
        strat_d2_val *= (1.0 + strat_d2_daily_ret)
    ...
    results.append({
        ...
        'strat_d1_roi': strat_d1_val - 100.0,
        'strat_d2_roi': strat_d2_val - 100.0,
        'total_runs': len(runs)
    })
